...
char ver[] = "version Michel du 06_03_2018";      // MM correction de l'avance à la volée avec sélecteur en A1 (réglage du pas par "PasCorecAv")
                                                  ...
                                                  // MM Temps de charge bobine majoré en cas d'accélération
...
const int TchargeBob = 7000;                 // temps de recharge bobine, ex 7 ms bobines Lucas T140, 5 ms à 14,5 V U batterie pour bobine Bosch crayon (mesuré sur Clio 1.6L 16S)
...
const float KmajorTchargBob = 0.2;           // MM Coefficient de majoration du temps de charge bobine en phase d'accélération
...
const int Multi = 0;                         // 1 pour multi-étincelles, 0 pour mettre HS
...
{ 
  ...
  tcor = 385;                                  // Correction temps de calcul (mesure faite à l'oscillo à pleine avance)
}
...
//   Davant_TchargeBob = 2 * T - Tprec - DureeAlimBob;  // Tenir compte des variations de régime moteur

// if (T < (Tprec * 0.9))                            // MM Si T est plus court (10% au moins) phase d'accélération
// {
...
// }
//
//  Davant_TchargeBob =  T - TchargeBobMajor;        // MM essai sans tenir compte des variations de régime moteur (peu de différence) avec majoration temps de charge
...
///////////////////////// Debug_printMM //////////////////
 if (Debug_printMM) {
  ...
  AvanceMoteur = (AngleCapteur - (Tempsecoule / Delaideg)) * 1.004;   // MM calcul avance totale à l'allumage du moteur, avance moteur doit être du type float, correction x 1.005
