#include <TimerOne.h>                             // Génère une interruption toutes les TchargeBob ou DsecuBob µs, MM pas nécessaire ici en CDI

//**************************************************************
                                                  // MM ATTENTION version CDI allumage à décharge de condensateur, pas de gestion du temps de charge bobine c'est inutile, le condensateur est chargé en permanence
                                                  // MM version Me: Multi-Etincelles, Ub: Ubatterie surveillance, Nk: comptage nombre de tours pour le démarrage au kick, C72 : cible de 72°
                                                  // Version dérivée des AEPL & AECP Multi Etincelles Nano. Allumage électronique programmable - Arduino
char ver[] = "version Michel CDI du 03_03_2018";  // MM correction de l'avance à la volée avec sélecteur en A1 (réglage du pas par "PasCorecAv")
                                                  // MM allumage à 0° d'avance au démarrage au kick (fin de cible au PMH), Temps d'avance non calculé jusqu'à Nplancher (500 tr/mn par exemple)
                                                  ...
                                                  // MM remise à 0 de N (comptage des tours moteurs) si le moteur n'a pas démarré ou a calé, pour un nouveau démarrage (au kick)
...
int tFlash = 100;      // Durée du flash en µs 100, typique
           ...
           //************Ces valeurs sont eventuellement modifiables*****************
                                             // Ce sont Nkick Nplancher, T_CDI, TchargeCDI, Ndem, PasCorecAv
...
const int Nplancher = 500;                   // vitesse en t/mn jusqu'à laquelle l'avance est fixe à 0°, au-dessus l'avance est celle programmée et avec correction
const int T_CDI = 500;                       // temps d'étincelle en CDI, en uS
const int TchargeCDI = 500;                  // temps charge condensateur en CDI, en uS
...
const int EtinMulti = 4;                     // Nombre d'étincelles après la principale en mode multi-étincelles
const float CoefMulti = 1;                   // Coefficient (0 à 1) appliqué aux temps de CDI lors d'un multi-étincelles, le temps est plus court que pour la première étincelle
...
unsigned long Tdem = 0;             // Période correspondante à Ndem, forcée pour le premier tour
...
int UneEtin = 0;                    // = 1 pour chaque étincelle, testé et remis à zero
...
{ 
  ...
  tcor = 285;                                  // ************** Correction temps de calcul ************* (mesure des temps ou de l'avance réelle pour compenser le temps de calcul du programme)
}                                              // Temps de compensation mesuré à l'oscilloscope à environ 5 000 tr/mn avance au maximum, pour une bonne précision
...
{
 Dfixe = (T * (AngleCapteur - AngAvfixe) / 360) - T_CDI;      // MM D fixe avec avance fixe à par exemple 5° entre Ndem et Nplancher Dfixe de 55,55 ms à 6666,667 ms
...
void  Etincelle ()       ////////// Décharge du condensateur pour créer l'étincelle d'allumage
{
  digitalWrite(Bob1, 1);                     // Décharge du condensateur d'allumage, donc étincelle ************* ALLUMAGE ***************
  digitalWrite(Bob2, 1);                     // Décharge du condensateur d'allumage, donc étincelle 
  Stop_temps_Etincelle = micros();           // MM Calcul des temps  *************** CALCUL TEMPS ****************
  UneEtin = 1;                               // MM indication qu'il a eu une étincelle d'allumage

  digitalWrite(Flash, 1);                    // Flash stroboscope à leds
  delayMicroseconds(tFlash);                 // Temps nécessaire pour voir l'éclair de la LED Flash stroboscope
  digitalWrite(Flash, 0);

  delayMicroseconds(T_CDI - tFlash);         // Attendre fin d'étincelle CDI T en µs - le temps de flash
  digitalWrite(Bob1, 0);                     // MM CDI autoriser la recharge du condensateur d'allumage
  digitalWrite(Bob2, 0);                     // MM CDI autoriser la recharge du condensateur d'allumage 
  
  if (Multi && (T >= T_multi))Genere_multi(); // si on a multi étincelles et T supérieur à temps multi (régime maxi du multi) on génère multi éticelles
}
...
{                                            // L'étincelle principale a juste été générée
  ...
  delayMicroseconds (TchargeCDI);            // Attendre le temps de charge normal du condensateur CDI, T en µs
  
  while (N_Etin < EtinMulti)                 // Tant que le seuil du nombre d'étincelles secondaires n'est pas atteint :
  {
  digitalWrite(Bob1, 1);                     // Décharge du condensateur d'allumage, donc étincelle ************* ALLUMAGE ***************
  digitalWrite(Bob2, 1);                     // Décharge du condensateur d'allumage, donc étincelle 
  delayMicroseconds((T_CDI) * CoefMulti);    // Recharger x par un coefficient, temps plus court si besoin
  digitalWrite(Bob1, 0);                     // MM CDI autoriser la recharge du condensateur d'allumage
  digitalWrite(Bob2, 0);                     // MM CDI autoriser la recharge du condensateur d'allumage
  delayMicroseconds ((TchargeCDI) * CoefMulti); // Attendre le temps de charge du condensateur CDI T en µs x par un coefficient, temps plus court si besoin
...
void Ubatterie()    ///////////// MM Surveillance tension batterie
{
   tension = analogRead(Vbatterie);    // Lecture tension batterie pour contrôle par LEDs
//   pc(tension)
   float Ubat = tension / 68.286;      // tension de 0 à 1023 vers U batterie de 0 à 15 V, utilisé pour serial print U bat
...
void loop()    //////////////// Attendre les tops capteur pour générer l'allumage
{
 ...
 UneEtin = 0;                                   // Remise à 0 du compteur Une étincelle
...
//   int Degdepcal = analogRead(Depression);     // Pour calibrage du capteur MVP5050 
  ...
  if (T <= Tplancher && T > Tlim && N > Nkick)   // Si le régime moteur est au dessus du régime de démarrage électrique N plancher et sous le régime maxi (zone rouge) et N supérieur au seuil Nkick
  { 
    CalcD();                                     // calcul du temps d'attente entre début de la cible et l'allumage, calcul de l'avance à l'allumage
    Etincelle();                                 // Etincelle d'allumage
  }

  if (T <= Tlim)                                 // Si le régime moteur est égal ou supérieur au régime maxi (zone rouge)
  {
   digitalWrite(Bob1, 0);                        // Coupure allumage
   digitalWrite(Bob2, 0);                        // Coupure allumage
  }
...
//  Tprec = T;                                   // Sauve la valeur de T en cas de perte d'info du capteur
  ...
  if (N <= Nkick || (UneEtin == 0 && T >= Tlim)) // MM Si N inférieur au seuil Nkick, ou s'il n'y a pas eu d'étincelle sous le régime maxi
  {
    Etincelle();                                 // MM étincelle (multiple) en fin de la cible à 0° au PMH du moteur
  }   
  
  digitalWrite(Led,LOW);                         // MM extinction Led de calage statique
  Correction_Avance();                           // MM correction avance en fonctionnement
  Ubatterie();                                   // Mesurer la tension batterie
...
///////////////////////// Debug_printMM //////////////////
 if (Debug_printMM) {
  ...
  AvanceMoteur = (AngleCapteur - (Tempsecoule / Delaideg))* 1.012;  // MM calcul avance totale à l'allumage du moteur, avance moteur doit être du type float, correction coeff
